...
namespace detail
{
    ...
    struct serialize
    {
        ...
        static void apply(Writer& writer, const T& value)
        {
            ...
            writer.begin_object(value);
                ctti::meta::foreach2<Model>(loop_body);
    ...
    struct deserialize<T, true, true>
    {
        ...
        struct callback
        {
            ...
            {
                T output_value{};
                ctti::meta::foreach2<Model>(loop_body<InputValue>{&output_value, input_value});
                return output_value;
            }
        };
...
struct json_formatter
{
    ...
    {
        out.write("(");
        ctti::meta::foreach2<std::tuple<Ts...>>(tuple_loop_body_t<Output, Ts...>{&tuple, this, &out});
        out.write(")");
    }
